...
# create model
config = ModelArgs(n_layers=24, n_heads=12, dim=768, vocab_size=50304, max_seq_len=1024, n_kv_heads=6, intermediate_size=4*768)
model = LlamaTransformer(config)
...
if is_fsdp:
    model = fsdp.FullyShardedDataParallel(model.cuda(fsdp_local_rank))
...
min_lr = max_lr * 0.1
warmup_steps = 715 * 2
max_steps = 19073 * 2 # 19,073 steps is ~1 epoch, if data is 10B tokens and batch size 0.5M tokens
def get_lr(it):
...
log_dir = f"log_tp_{config.n_layers}_{config.n_heads}_{config.n_kv_heads}_{max_lr:.0e}"
